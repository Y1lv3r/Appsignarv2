...
<?php include("./cabeceras/cabecera_Festivos.php") ?>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title m-0 font-weight-bold text-primary" id="exampleModalLabel">FESTIVOS</h5>

                            </div>
                            <div class="modal-body">
                                <div class="form-row">
                                    <div class="form-group col-md-12">
                                        <label class="m-0 font-weight-bold text-primary">Ingrese el día festivo: </label>
                                        <input autofocus="autofocus" class="form-control w-full px-4 py-2 border rounded-md dark:bg-darker dark:border-gray-700 focus:outline-none focus:ring focus:ring-primary-100 dark:focus:ring-primary-darker" type="date" name="Festivo_dia" required />
                                    </div>
                                </div>
                            </div>
...
if (isset($_GET["variable2"])) {
    echo "<script>
            ...
            alertify.error('Este día ya esta agregado, por favor elegir otro');
            </script>";
?>
    <script>
        var stateObj = {
        ...
        history.pushState(stateObj, "page 2", "festivos");
    </script>
    <?php
} else {
    if (isset($_GET["variable"])) {
        $condicion = $_GET['variable'];
        if (empty(!$condicion)) {
            $condicion = "mostrar";
...
<?php
$sentenciaFestivo = $bdd->prepare("SELECT * FROM dias_festivos");
$sentenciaFestivo->execute();
$listaFestivo = $sentenciaFestivo->fetchAll(PDO::FETCH_ASSOC);
...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h4 class="m-0 font-weight-bold text-primary" class="label" align="center">Tabla Festivos</h4>
            <?php if ($rol['Rol'] == $rol1) { ?>
                <a href="./modulos/exportar_excel_festivos.php"> <button style="position: absolute;top: 0.3cm;left: 88%; z-index: 2;" class="btn btn-primary">EXCEL&nbsp;&nbsp;&nbsp;<i class="fa fa-download" aria-hidden="true"></i></button></a>
            <?php } ?>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table display nowrap" id="example" class="display" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        <?php foreach ($listaFestivo as $proyecto) { ?>
                            <tr>
                                <td><?php echo $proyecto['fecha'] ?></td>
                                <td><?php echo $proyecto['dia'] ?></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
