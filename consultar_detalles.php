...
<?php include("modulos/detalles.php") ?>
<?php include("./cabeceras/cabecera_Asig_Detalle.php") ?>
...
$t = $pdo->prepare("SELECT totalhoras from hora_laboral ");
$t->execute();
$e = $t->fetch(PDO::FETCH_LAZY);
$jornada = '';
$jornada = $e['totalhoras'];
...
$sentenciaA = $pdo->prepare("SELECT start,SEC_TO_TIME(TIME_TO_SEC(start)) as hora_inicio,SEC_TO_TIME(TIME_TO_SEC(end)) as hora_fin,
                 TIMEDIFF(end,start) as horas_asignadas_total_dia,
                  month(start) as mes,semana,dia, empleado,title,lugar,estado,division_e,perfil_e,division,perfil,area FROM `events` WHERE empleado LIKE '%$nombre%' and semana <> 0 GROUP by empleado,semana,start");
/*$sentenciaA = $pdo->prepare("
    ...
    A1.lugar as Lugar 
FROM events A1
WHERE Date_Format(A1.start, '%Y/%m/%d') >= '2022-01-01' AND A1.empleado != 'Administrador' 
...
Left Join (SELECT 
           		...
           		(100000)-sum(TIMEDIFF(A0.end,A0.start)) as horasLibres,
           		A0.empleado , 
...
WHERE Date_Format(B0.start, '%Y/%m/%d') >= '2022-01-01' AND B0.empleado != 'Administrador'
Group By B0.title, B0.empleado,Date_Format(B0.start, '%Y/%m/%d')
ORDER BY Fecha ASC;");*/
$sentenciaA->execute();
$listaAsignacionp = $sentenciaA->fetchAll(PDO::FETCH_ASSOC);
$sentencia = $pdo->prepare("SELECT * FROM empleados WHERE Nombre != 'Administrador' AND Nombre != 'Administrador2' AND Nombre != 'Administrador3' AND estado != 'inactivo' AND Division != ''");
$sentencia->execute();
    ...
    <div class="panel with-nav-tabs panel-default">
        ...
        <div class="panel-body">
            <div class="tab-content">
                ...
                <!-- <select class="form-control mb-2 mr-sm-2 form-control col-md-3" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                        <option value="">Habilitar fin de semana</option>
                        <option value="consultar_finde">Si</option>
                ...
                <ul class="nav nav-tabs">
                    <input type="search" name="emple" id="emple" list="empleados" class="custom-select form-control mb-2 mr-sm-2 bg-light col-md-3" placeholder="Nombre..." required>
                    ...
                    <div class="form-group col-md-3">
                        <input type="button" class="btn btn-primary" name="enviar" value="Consultar" href="javascript:;" onclick="Hola($('#emple').val(),$('#mensaje').val());">
                    </div>
                </ul><br>

                <script>
                    function Hola(emple, mensaje) {
                        var parametros = {
                            "emple": emple,
                            "Mensaje": mensaje
                        };
                        $.ajax({
                            data: parametros,
                            url: 'modulos/mostrar.php',
                            type: 'post',
                            beforeSend: function() {
                                $("#resultado").html("<h1>Procesando, espere por favor...</h1>");
                            },
                    ...
                    <div class="col-md-12">
                        <div class="card with-margin card_shadow card_border rounded">
                            <div class="container-fluid" id="mainContainer" style="height: 50%;">
                                ...
                                <a href="./modulos/exportar_excel.php"> <button style="position: absolute;top: 2.5cm;left: 79.5%; z-index: 2;" class="btn btn-secondary">EXCEL&nbsp;&nbsp;&nbsp;<i class="fa fa-download" aria-hidden="true"></i></button></a>
                                <button style="position: absolute;top: 2.5cm;left: 89%; z-index: 2; " tabindex="-1" data-toggle="dropdown" class="btn btn-secondary dropdown-toggle" type="button">
                                            ...
                                            <div class="card-body">
                                                            ...
                                                            <div class="cajaboton">
                                                                <div class="listacolumnas" tabindex="-1">
                                                                    <form name="tcol" onsubmit="return false">
                                                                        ...
                                                                        <div class="checkbox">
                                                                            <input type=checkbox name="col6" id="6" onclick="toggleVis(this.name)" checked> <label class="form-check-label" for="6">Estado</label>
                                                                        </div>
                                                                        <div class="checkbox">
                                                                            <input type=checkbox name="col7" id="7" onclick="toggleVis(this.name)" checked> <label class="form-check-label" for="7">Horas Asignadas</label><br>
                                                                        </div>
                                                                        <div class="checkbox">
                                                                            <input type=checkbox name="col8" id="8" onclick="toggleVis(this.name)" checked> <label class="form-check-label" for="8">Día</label>
                                                                        </div>
                                                                        <div class="checkbox">
                                                                            <input type=checkbox name="col9" id="9" onclick="toggleVis(this.name)" checked> <label class="form-check-label" for="9">Hora Inicio</label>
                                                                        </div>
                                                                        <div class="checkbox">
                                                                            <input type=checkbox name="col10" id="10" onclick="toggleVis(this.name)" checked> <label class="form-check-label" for="10">Hora Fin</label><br>
                                                                        </div>
                                                                        <div class="checkbox">
                                                                            <input type=checkbox name="col11" id="11" onclick="toggleVis(this.name)" checked> <label class="form-check-label" for="11">Lugar</label><br>
                                                                        </div>
                                                                        <div class="checkbox">
                                                                            <input type=checkbox name="col12" id="12" onclick="toggleVis(this.name)" checked> <label class="form-check-label" for="12">Division_empleado</label><br>
                                                                        </div>
                                                                        <div class="checkbox">
                                                                            <input type=checkbox name="col13" id="13" onclick="toggleVis(this.name)" checked> <label class="form-check-label" for="13">Perfil_empleado</label><br>
                                                                        </div>
                                                                    </form>
                                                                </div>
                                                            </div>
                                                ...
                                                <div class="table-responsive">
                                                    <table class="table display nowrap" id="example" class="display" cellspacing="0" width="100%">
                                                        ...
                                                        <tbody>
                                                            <?php foreach ($listaAsignacionp as $empleado) { ?>
                                                                <tr>
